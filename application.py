...
SYSTEM_PROMPT = f"""Tu es un expert-comptable spécialisé dans l'accompagnement des maisons d'édition indépendantes françaises, au sein du cabinet {NOM_CABINET}.
...
# =====================
    ...
    **✍️ Droits d'auteurs → 📒 Réel (comptabilisé)** détecte automatiquement la correspondance ISBN → auteur
    ...
    if st.button("⚙️ Générer le socle analytique", type="primary"):
        ...
        pivot = normaliser_codes_isbn(pivot, "Code_Analytique")
        ...
        if nb_avant and nb_apres < nb_avant:
            st.info(f"ℹ️ {nb_avant - nb_apres} code(s) analytique(s) fusionné(s) automatiquement "
                    f"(même numéro ISBN, libellé légèrement différent d'une ligne à l'autre).")
...
#   elif page == "📄 Rapport de pilotage":
    ...
    with col1:
        ...
        nom_cabinet   = st.text_input("Nom du cabinet", value=NOM_CABINET)
